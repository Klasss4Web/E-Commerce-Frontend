export const USER_LOGIN_REQUEST = "USER_LOGIN_REQUEST";
export const USER_LOGIN_SUCCESS = "USER_LOGIN_SUCCESS";
export const USER_LOGIN_FAILURE = "USER_LOGIN_FAILURE";
export const USER_LOGOUT = "USER_LOGOUT";

//REGISTER
export const USER_REGISTER_REQUEST = "USER_REGISTER_REQUEST";
export const USER_REGISTER_SUCCESS = "USER_REGISTER_SUCCESS";
export const USER_REGISTER_FAILURE = "USER_REGISTER_FAILURE";

//ADMIN CREATE USERS
export const ADMIN_REGISTER_USER_REQUEST = "ADMIN_REGISTER_USER_REQUEST";
export const ADMIN_REGISTER_USER_SUCCESS = "ADMIN_REGISTER_USER_SUCCESS";
export const ADMIN_REGISTER_USER_FAILURE = "ADMIN_REGISTER_USER_FAILURE";

//PROFILE
export const USER_PROFILE_REQUEST = "USER_PROFILE_REQUEST";
export const USER_PROFILE_SUCCESS = "USER_PROFILE_SUCCESS";
export const USER_PROFILE_FAILURE = "USER_PROFILE_FAILURE";
export const USER_PROFILE_UPDATE= "USER_PROFILE_UPDATE";

//UPDATE PROFILE
export const USER_UPDATE_PROFILE_REQUEST = "USER_UPDATE_PROFILE_REQUEST";
export const USER_UPDATE_PROFILE_SUCCESS = "USER_UPDATE_PROFILE_SUCCESS";
export const USER_UPDATE_PROFILE_FAILURE = "USER_UPDATE_PROFILE_FAILURE";
export const USER_UPDATE_PROFILE_RESET = "USER_UPDATE_PROFILE_RESET";


//ADMIN UPDATE PROFILE
export const ADMIN_UPDATE_PROFILE_REQUEST = "ADMIN_UPDATE_PROFILE_REQUEST";
export const ADMIN_UPDATE_PROFILE_SUCCESS = "ADMIN_UPDATE_PROFILE_SUCCESS";
export const ADMIN_UPDATE_PROFILE_FAILURE = "ADMIN_UPDATE_PROFILE_FAILURE";
export const ADMIN_UPDATE_PROFILE_RESET = "ADMIN_UPDATE_PROFILE_RESET";


//GET ALL USERS: ONLY ADMINS CAN GET ALL USERS
export const GET_USERS_REQUEST = "GET_USERS_REQUEST";
export const GET_USERS_SUCCESS = "GET_USERS_SUCCESS";
export const GET_USERS_FAILURE = "GET_USERS_FAILURE";
export const GET_USERS_RESET = "GET_USERS_RESET";

//UPDATE USERS STATUS: ONLY ADMINS CAN BAN/UNBAN USERS
export const ADMIN_UPDATE_USER_STATUS_REQUEST = "ADMIN_UPDATE_USER_STATUS_REQUEST";
export const ADMIN_UPDATE_USER_STATUS_SUCCESS = "ADMIN_UPDATE_USER_STATUS_SUCCESS";
export const ADMIN_UPDATE_USER_STATUS_FAILURE = "ADMIN_UPDATE_USER_STATUS_FAILURE";

//DELETE USERS: ONLY ADMINS CAN DELETE USERS
export const ADMIN_DELETE_USER_STATUS_REQUEST = "ADMIN_DELETE_USER_STATUS_REQUEST";
export const ADMIN_DELETE_USER_STATUS_SUCCESS = "ADMIN_DELETE_USER_STATUS_SUCCESS";
export const ADMIN_DELETE_USER_STATUS_FAILURE = "ADMIN_DELETE_USER_STATUS_FAILURE";
